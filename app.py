...
if st.button("🔄 Generate Comparison", type="secondary"):
    with st.spinner("Regenerating original dataset and making predictions..."):
        ...
        if st.checkbox("Show full comparison table (all 200 matches)"):
            full_display = comparison_df[display_cols]
            full_display.columns = ['P1 Rank', 'P2 Rank', 'P1 Win Rate', 'P2 Win Rate',
                                   'Court', 'Weather', 'Actual Winner', 'Predicted Winner',
                                   'Confidence', 'Correct?']
            st.dataframe(
                full_display.style.apply(highlight_correct, axis=1),
                use_container_width=True,
                height=600
            )
